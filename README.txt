...
`generate_shorts.py` 스크립트는 `data/image` 폴더의 이미지를 이용해 9:16 세로 비디오를 생성합니다. 실행하면 `song` 폴더에 있는 음악 중 하나를 무작위로 배경음악으로 선택하고, 결과물은 `results` 폴더에 `YYYYMMDD_HHMM.mp4` 형식으로 저장됩니다.

필요한 파이썬 패키지(`moviepy`)가 설치되어 있어야 하며, 다음과 같이 실행할 수 있습니다:
